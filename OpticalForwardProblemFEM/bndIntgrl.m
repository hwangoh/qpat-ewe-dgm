function [R_int,b_int] = bndIntgrl(Mesh,E,i,I_s,Lght_ElmtInd)
...
if size(Mesh.Dimensns,2) == 2;
    H = {};
    H{1} = find(E(:,2)==min(Mesh.Nodes(:,2)));
    H{2} = find(E(:,1)==min(Mesh.Nodes(:,1)));
    H{3} = find(E(:,1)==max(Mesh.Nodes(:,1)));
    H{4} = find(E(:,2)==max(Mesh.Nodes(:,2)));
    ...
    %=== R matrix ==%
    Jac = norm(E(EdgNode(1),:) - E(EdgNode(2),:),2);
