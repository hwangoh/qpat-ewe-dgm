function [SampleMeanT11,SampleMeanT22,SampleMeanT12,SampleMeanvx,SampleMeanvy,SampleCovT11,SampleCovT22,SampleCovT12,SampleCovvx,SampleCovvy]=EWE_DGM2D_AE_ConstructStats_FromAbsorptionCoeff(RunOptions,MeshANodes,MeshAElements,MeshADimensns,MeshINodes,MeshIElements,xA,yA,NpA,KA,PrecomputedIntrplteObjectsA,pinfoA,xI,yI,NpI,KI,PrecomputedIntrplteObjectsI,pinfoI,SensorsA,SensorsI,dt,Prior,PLOT)
...
% for errors arising from discretization.
%
% Inputs:
%   RunOptions:
%              NumberofTimeSteps
...
[~,~,~,~,mu_aAS_FEM]=SmoothnessPrior_Informative(MeshANodes,MeshAElements,2*MeshADimensns,Prior.Bounds_mu_a,Prior.Corr_mu_a,Prior.Exp_mu_a,Prior.N_Samples,PLOT);

%=== Initial pressure Samples ===%
for ii=1:N_Samples;
%absorption coefficient    
    PrmtrsDS.mu_a = mu_aAS_FEM(:,ii);
    PrmtrsDS.mu_a_elmts = reshapeNodes(MeshD,PrmtrsDS.mu_a);
%scattering coefficient    
    PrmtrsDS.mu_s = mu_sS(:,ii);
    PrmtrsDS.mu_s_elmts = reshapeNodes(MeshD,PrmtrsDS.mu_s);    
%diffusion coefficient
    PrmtrsDS.kappa = zeros(MeshD.N_Nodes,1);
    for j=1:MeshD.N_Nodes;
        PrmtrsDS.kappa(j) = 1/(2*(PrmtrsDS.mu_a(j) + ((1-PrmtrsPrp.g)*PrmtrsDS.mu_s(j))));
    end
    PrmtrsDS.kappa_elmts = reshapeNodes(MeshD,PrmtrsDS.kappa);
%Light fluence
    for j=1:N_Illum;
        [phiSolved,~,~,~,~,~] = ODTForward(MeshD,PrmtrsDS,Data.IllumPattern(j,:));
        phiDStemp(:,j) = phiSolved;
%Initial pressure distribution
        p0DStemp(:,j) = Data.T*((PrmtrsDS.mu_a).*(phiDStemp(:,j)));
    end
    p0DS(:,ii) = p0DStemp(:);
end
...
if PLOT.PriorSamples==1;
    figure(PLOT.Figure_PriorSamples)
