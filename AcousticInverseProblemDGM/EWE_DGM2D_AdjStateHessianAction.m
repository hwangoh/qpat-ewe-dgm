function HAOutput = EWE_DGM2D_AdjStateHessianAction(RunOptions,SensorsI,FrwdFunction,hhat,hRecon,alphaRecon,L_ET11,L_ET22,L_ET12,L_Evx,L_Evy,T11AdjTime0,T22AdjTime0,Prior,MeshIElements,MeshIN_Elm,MeshINodes,MeshIN_Nodes,pinfoI,PrecomputedIntrplteObjectsI,xI,yI,NpI,KI,rhoI,dt,CurrentIteration,hRelError,PLOT,transp_flag)

% AdjStateHessianAction compute the action of the Hessian on the vector via
% the adjoint-state method. This circumvents explicitly constructing the
% Hessian.
%
% Inputs:
%   RunOptions:
...
%   h - Input vector of size N_Nodes with respect to FEM Mesh
%   hRecon - Reconstructed absorbed energy density. This is required when positivity transform is implemented, because the mapping is linearized around the current MAP estimate
%   alphaRecon - Reconstructed hyperprior parameter alpha. Use this for constructing uncertainty estimates
...
if RunOptions.EWE_LS_LogExpTransform == 1 
    ...
    if RunOptions.EWE_DGM2D_UseGaussNewton == 1
        HAOutput = alphaRecon*Direction_Prior - Direction_Data;
    end
