...
% Set optimized parameter, GNalpha, depending on order N
if (N<16)
  GNalpha = alpopt(N);
else
  GNalpha = 5/3;
...
% Combine blend & warp
warp1 = blend1.*warpf1.*(1 + (GNalpha*L1).^2);
warp2 = blend2.*warpf2.*(1 + (GNalpha*L2).^2);
warp3 = blend3.*warpf3.*(1 + (GNalpha*L3).^2);
